...
from intel_engine.db import create_engine_from_settings, init_schema_for_sqlite, sessionmaker_for_engine
from intel_engine.jobs import crawl_enabled_sources
from intel_engine.pipeline import reprocess_existing_items, run_pipeline_once, run_scheduler_once, run_worker_once
from intel_engine.settings import Settings
from intel_engine.source_seed import seed_sources_from_channel_configs
from intel_engine.storage import DEFAULT_DB_PATH, ItemRepository, create_engine_for_path, init_db


def build_crawl_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="运行公开信源抓取并写入 SQLite。")
    parser.add_argument("--channels-dir", type=Path, default=CHANNELS_DIR, help="频道配置目录。")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB_PATH, help="SQLite 数据库路径。")
    parser.add_argument("--channel", choices=["ai", "amazon"], default=None, help="只抓取指定频道。")
    return parser


def run_crawl_command(argv: Sequence[str] | None = None, client: httpx.Client | None = None) -> int:
...
def run_seed_sources_command(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"信源导入完成：新增 {stats.created} 个，更新 {stats.updated} 个，总数 {stats.total} 个。")
    return 0


def run_schedule_command(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="根据 source_states 生成生产抓取任务。")
    parser.add_argument("--now", default=None, help="ISO 8601 时间；默认当前 UTC 时间。")
    parser.add_argument("--limit", type=int, default=None, help="最多调度多少个信源。")
    args = parser.parse_args(argv)

    stats = run_scheduler_once(_production_sessionmaker(), now=_parse_now(args.now), limit=args.limit)
...
def run_reprocess_items_command(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="用当前 LLM Provider 重处理已有条目的中文标题、摘要和推荐理由。")
    parser.add_argument("--channel", choices=["ai", "amazon"], default=None, help="只处理指定频道。")
    parser.add_argument("--limit", type=int, default=10, help="最多处理多少个已有条目。")
    args = parser.parse_args(argv)

    stats = reprocess_existing_items(_production_sessionmaker(), channel=args.channel, limit=args.limit)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    crawl_parser = subparsers.add_parser("crawl", description="抓取公开信源并写入 SQLite。")
    crawl_parser.add_argument("--channels-dir", type=Path, default=CHANNELS_DIR, help="频道配置目录。")
    crawl_parser.add_argument("--db", type=Path, default=DEFAULT_DB_PATH, help="SQLite 数据库路径。")
    crawl_parser.add_argument("--channel", choices=["ai", "amazon"], default=None, help="只抓取指定频道。")
    seed_parser = subparsers.add_parser("seed-sources", description="导入生产信源 seed。")
    seed_parser.add_argument("--channels-dir", type=Path, default=CHANNELS_DIR, help="频道配置目录。")
    schedule_parser = subparsers.add_parser("schedule", description="生成生产抓取任务。")
    schedule_parser.add_argument("--now", default=None, help="ISO 8601 时间；默认当前 UTC 时间。")
    schedule_parser.add_argument("--limit", type=int, default=None, help="最多调度多少个信源。")
    worker_parser = subparsers.add_parser("worker", description="处理生产抓取任务。")
    worker_parser.add_argument("--worker-id", default="local-worker", help="worker 标识。")
    worker_parser.add_argument("--limit", type=int, default=10, help="最多领取多少个任务。")
    worker_parser.add_argument("--now", default=None, help="ISO 8601 时间；默认当前 UTC 时间。")
    pipeline_parser = subparsers.add_parser("pipeline-once", description="执行一次生产调度和 worker 闭环。")
    pipeline_parser.add_argument("--worker-id", default="local-worker", help="worker 标识。")
    pipeline_parser.add_argument("--limit", type=int, default=10, help="最多处理多少个任务。")
    pipeline_parser.add_argument("--now", default=None, help="ISO 8601 时间；默认当前 UTC 时间。")
    reprocess_parser = subparsers.add_parser("reprocess-items", description="用当前 LLM Provider 重处理已有条目。")
    reprocess_parser.add_argument("--channel", choices=["ai", "amazon"], default=None, help="只处理指定频道。")
    reprocess_parser.add_argument("--limit", type=int, default=10, help="最多处理多少个已有条目。")

    args = parser.parse_args(argv)
    if args.command == "crawl":
        crawl_args: list[str] = ["--channels-dir", str(args.channels_dir), "--db", str(args.db)]
        if args.channel:
