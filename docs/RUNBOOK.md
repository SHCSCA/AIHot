...
- 生产 timer 每小时扫描一次、单轮最多处理 80 个到期任务，12 小时容量上限为 960 个任务，高于当前 665 个启用信源；已有活跃任务会在应用批量上限前排除，避免失败任务阻塞后续信源。
...
- 部署先同步等待 `aihot-pipeline@1.service` 完成一次单任务 smoke；通过后才启用每小时触发 `aihot-pipeline@80.service` 的 timer。
